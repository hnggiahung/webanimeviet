...
import { Search, Moon, Sun, Menu, X, Star, Play, ChevronRight, Clock, Heart, Loader2, AlertTriangle, Bell, TrendingUp } from "lucide-react";
...
/* ═══ MINI CARD LIST - Modern App-style ═══ */
.mobile-homepage .mini-card-item {
  display: flex;
  align-items: center;
  gap: 14px;
  padding: 12px 14px;
...
.mobile-homepage .mini-card-item:active {
  background: #1a1a2e;
  transform: scale(0.98);
}
.mobile-homepage .mini-card-thumb {
  width: 56px;
  height: 80px;
  border-radius: 8px;
  overflow: hidden;
...
.mobile-homepage .mini-card-info {
  flex: 1;
  min-width: 0;
  display: flex;
...
.mobile-homepage .mini-card-title {
  font-size: 13px;
  font-weight: 600;
  color: #f0f0f0;
  line-height: 1.4;
...
.mobile-homepage .mini-card-meta {
  font-size: 11px;
  color: #8a8a9a;
  display: flex;
  align-items: center;
...
.mobile-homepage .mini-card-rating {
  display: flex;
  align-items: center;
  gap: 3px;
  color: #f5c518;
...
.mobile-homepage .mini-card-complete {
  background: #16a34a20;
  color: #4ade80;
  font-size: 10px;
  font-weight: 700;
...
.mobile-homepage .mini-card-number {
  width: 28px;
  flex-shrink: 0;
  text-align: center;
  font-size: 16px;
...
.mobile-homepage .mini-card-number.gold { color: #f5c518; }
.mobile-homepage .mini-card-number.silver { color: #c0c0c0; }
.mobile-homepage .mini-card-number.bronze { color: #cd7f32; }
.mobile-homepage .mini-card-number.default { color: #555; }
`;
    ...
    <header className="sticky top-0 z-50 bg-[#0a0a12]/95 backdrop-blur-xl border-b border-[#1f1f2e]">
      <div className="flex items-center justify-between px-3 h-12 gap-2">
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setDark(!dark)} className="p-2 text-gray-400 hover:text-gray-200 hover:bg-[#1f1f2e] rounded-full transition-all">
            {dark ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
          </button>
          <button onClick={() => setMenuOpen(!menuOpen)} className="p-2 text-gray-400 hover:text-gray-200 hover:bg-[#1f1f2e] rounded-full transition-all">
        ...
        <div className="absolute top-full left-0 right-0 bg-[#0a0a12]/98 backdrop-blur-xl border-b border-[#1f1f2e] shadow-2xl z-50">
          <div className="px-4 py-4 space-y-3">
            ...
            <div className="border-t border-[#1f1f2e] pt-3 mt-3">
              <button className="w-full bg-[#e50914] hover:bg-[#b91c1c] text-white text-sm font-bold py-2.5 rounded-lg transition-colors">Đăng nhập</button>
            </div>
          </div>
        </div>
      )}
...
// ─── SECTION HEADER ──────────────────────────────────────────────────────────
function SectionHeader({ icon: Icon, title, color = "#e50914" }) {
  return (
...
function TopHorizontalScroll({ items, onMovieClick }) {
  ...
  return (
    <div className="px-3 py-3">
      <div className="mb-3"><SectionHeader title="Đề Cử" /></div>
      <div className="smooth-scroll-x flex gap-2.5 pb-1 scrollbar-hide">
        {items.slice(0, 15).map((item, idx) => {
          ...
          return (
            <div key={item._id || item.slug || idx} className="flex-shrink-0 cursor-pointer" style={{ width: 90 }} onClick={() => onMovieClick && onMovieClick(slug)}>
              <div className="relative overflow-hidden" style={{ width: 90, height: 130, borderRadius: 10 }}>
                ...
                {isComplete ? <div className="complete-badge">HOÀN TẤT</div> : ep ? <div className="ep-badge">TẬP {ep.slice(0, 4)}</div> : null}
              </div>
...
function HeroSlider({ items, onMovieClick }) {
  ...
  return (
    <div className="px-3 mb-4">
      <div className="relative w-full overflow-hidden rounded-2xl bg-[#1a1c20] select-none" style={{ aspectRatio: "16/9", maxHeight: 280 }}
        onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
        {slides.map((slide, index) => (
          <div key={slide.slug || index} className={`absolute inset-0 w-full h-full transition-opacity duration-500 ease-in-out ${index === currentIndex ? "opacity-100 z-[2]" : "opacity-0 z-[1]"}`}>
            <img src={slide.image} alt={slide.title} className="absolute inset-0 w-full h-full object-cover object-center"
              loading={index === currentIndex ? "eager" : "lazy"}
              onError={(e) => { e.target.style.display = "none"; e.target.parentElement.style.background = "#1a1c20"; }} />
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent z-10" />
            <div className="absolute inset-0 bg-gradient-to-r from-black/70 to-transparent z-10" />
          </div>
        ))}
        <div className="absolute bottom-0 left-0 right-0 z-20 p-4 pb-6">
          <h2 className="text-white font-bold text-lg leading-tight mb-1.5 anim-title line-clamp-2">{currentSlide.title}</h2>
          <div className="flex items-center flex-wrap gap-x-2 gap-y-1 mb-2 anim-info">
            ...
            {currentSlide.episode && <span className="text-gray-300 text-[10px] flex items-center gap-1"><Clock className="w-3 h-3 text-gray-400" />{currentSlide.episode}</span>}
            {currentSlide.year && <span className="text-gray-400 text-[10px]">{currentSlide.year}</span>}
            <span className="bg-green-600 text-white font-bold text-[8px] px-2 py-0.5 rounded uppercase tracking-wider">{currentSlide.quality}</span>
          </div>
          {currentSlide.genres.length > 0 && (
            <div className="flex items-center gap-1.5 mb-2 anim-info flex-wrap">
              {currentSlide.genres.slice(0, 3).map((g, i) => <span key={i} className="text-[9px] text-green-400 bg-green-500/10 px-1.5 py-0.5 rounded">{g}</span>)}
            </div>
          )}
          <div className="text-[10px] text-gray-400 mb-2.5 anim-info">Studio: <span className="text-gray-300">{currentSlide.studio}</span></div>
          <div className="flex items-center gap-2 anim-btns">
            <button onClick={() => onMovieClick && onMovieClick(currentSlide.slug)}
              ...
              <Play className="w-3.5 h-3.5 fill-white" /> Xem Phim
            </button>
        ...
        {totalSlides > 1 && (
          <div className="absolute bottom-1 right-4 z-20 flex items-center gap-1.5">
            {slides.map((_, index) => (
              <button key={index}
                className={`transition-all duration-300 ${index === currentIndex ? "w-5 h-1.5 rounded-full bg-[#e50914]" : "w-1.5 h-1.5 rounded-full bg-white/40"}`}
                onClick={() => goTo(index)} aria-label={`Slide ${index + 1}`} />
            ))}
          </div>
        )}
      </div>
...
// ─── MINI CARD LIST ──────────────────────────────────────────────────────────
function MiniCardList({ items, showRanking = false, onMovieClick }) {
  if (!items || items.length === 0) return null;
  return (
    <div className="px-3">
      {items.map((item, i) => {
        ...
        const rating = item.tmdb?.vote_average || item.rating || 0;
        const ep = item.episode_current || "";
        const epTotal = item.episode_total || "";
        const isComplete = epTotal && ep && parseInt(ep) >= parseInt(epTotal);
        const slug = item.slug || "";
        const year = item.year || "";
        const rankClass = i === 0 ? "gold" : i === 1 ? "silver" : i === 2 ? "bronze" : "default";
        return (
          <div key={item._id || item.slug || i} className="mini-card-item" onClick={() => onMovieClick && onMovieClick(slug)}>
            {showRanking && <div className={`mini-card-number ${rankClass}`}>{i + 1}</div>}
            <div className="mini-card-thumb">
              <LazyImg src={src} alt={title} className="w-full h-full" />
              {rating > 0 && (
                <div className="rating-ribbon" style={{ padding: "2px 4px 5px 3px", fontSize: "7px" }}>
                  <Star className="star-icon" style={{ width: 6, height: 6 }} />
                  <span>{typeof rating === "number" ? rating.toFixed(1) : rating}</span>
                </div>
              )}
            </div>
            <div className="mini-card-info">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                {isComplete ? (
                  <span className="mini-card-complete">HOÀN TẤT</span>
                ) : ep ? (
                  <span className="mini-card-ep">TẬP {ep}{epTotal ? `/${epTotal}` : ''}</span>
                ) : null}
              </div>
            </div>
        ...
        {FILTER_TABS.map((tab) => (
          <button key={tab.key} onClick={() => onTabChange(tab.key)}
            className={`flex-1 text-xs py-2 px-2 rounded-lg font-medium transition-all ${
              activeTab === tab.key ? "text-white bg-[#e50914]/20 border-b-2 border-[#e50914] font-bold" : "text-gray-500 hover:text-gray-300"
            }`}>
            {tab.label}
          </button>
        ))}
...
function MovieGridCard({ anime, onMovieClick }) {
  ...
  return (
    <div className="cursor-pointer group" onClick={() => onMovieClick && onMovieClick(slug)}>
      <div className="relative overflow-hidden" style={{ aspectRatio: "2/3", borderRadius: 10 }}>
        <LazyImg src={src} alt={title} className="w-full h-full" />
        {rating > 0 && (
          <div className="rating-ribbon"><Star className="star-icon" /><span>{typeof rating === "number" ? rating.toFixed(1) : rating}</span></div>
        )}
        {isComplete ? <div className="complete-badge">HOÀN TẤT</div> : ep ? <div className="ep-badge">TẬP {ep.slice(0, 4)}</div> : null}
      ...
      <p className="text-gray-200 text-xs font-medium mt-1.5 leading-tight line-clamp-2 group-hover:text-[#e50914] transition-colors">{title}</p>
    </div>
  );
}
...
export default function MobileHomepage({
  allItems, filteredAnime, filterTab, setFilterTab, onMovieClick,
  isLoading, isError, dark, setDark,
}) {
  ...
  // Latest updates - sorted by episode count
  const latestUpdates = useMemo(() => {
    if (!allItems || allItems.length === 0) return [];
    return [...allItems].sort((a, b) => (parseInt(b.episode_current) || 0) - (parseInt(a.episode_current) || 0)).slice(0, 10);
  }, [allItems]);

  // Hot weekly - sorted by rating
  const hotWeekly = useMemo(() => {
    if (!allItems || allItems.length === 0) return [];
    return [...allItems].sort((a, b) => ((b.tmdb?.vote_average || b.rating || 0) - (a.tmdb?.vote_average || a.rating || 0))).slice(0, 5);
  }, [allItems]);
  ...
  return (
    <div className="mobile-homepage min-h-screen bg-[#0a0a12] text-white font-sans">
      ...
      <MobileHeader dark={dark} setDark={setDark} searchQuery={searchQuery} setSearchQuery={setSearchQuery} menuOpen={menuOpen} setMenuOpen={setMenuOpen} />

      {isLoading && (
        <div className="bg-blue-900/30 py-1.5 text-center">
          <span className="text-blue-400 text-[10px] flex items-center justify-center gap-1.5"><Loader2 className="w-3 h-3 animate-spin" /> Đang đồng bộ dữ liệu...</span>
        </div>
      )}
      {isError && !isLoading && (
        <div className="bg-amber-900/30 py-1.5 text-center">
          <span className="text-amber-400 text-[10px] flex items-center justify-center gap-1.5"><AlertTriangle className="w-3 h-3" /> Đang dùng dữ liệu dự phòng</span>
        </div>
      )}
      ...
      <div className="mb-5 mt-2">
        <div className="px-3 mb-3"><SectionHeader icon={Bell} title="Anime Mới Cập Nhật" color="#e50914" /></div>
        <MiniCardList items={latestUpdates} onMovieClick={onMovieClick} />
      </div>

      {/* ═══ Hot Tuần - Mini Card List with Ranking ═══ */}
      <div className="mb-5">
        <div className="px-3 mb-3"><SectionHeader icon={TrendingUp} title="Hot Tuần" color="#f5c518" /></div>
        <MiniCardList items={hotWeekly} showRanking={true} onMovieClick={onMovieClick} />
      </div>
      ...
      <div className="px-3 pb-6">
        {displayItems.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            {displayItems.map((item, idx) => <MovieGridCard key={item._id || item.slug || idx} anime={item} onMovieClick={onMovieClick} />)}
          </div>
        )}
      </div>
